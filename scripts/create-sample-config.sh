...
if [ -z "$OPENAI_PROVIDER_ID" ]; then
    echo -e "${RED}ERROR: Failed to create OpenAI provider${NC}"
    echo "Response: $OPENAI_PROVIDER_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] OpenAI provider created (ID: $OPENAI_PROVIDER_ID)${NC}"
...
if [ -z "$AZURE_PROVIDER_ID" ]; then
    echo -e "${RED}ERROR: Failed to create Azure provider${NC}"
    echo "Response: $AZURE_PROVIDER_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] Azure provider created (ID: $AZURE_PROVIDER_ID)${NC}"
...
if ! echo "$GPT4_1_MODEL_RESPONSE" | grep -q '"key":"gpt-4.1"'; then
    echo -e "${RED}ERROR: Failed to create GPT-4.1 model${NC}"
    echo "Response: $GPT4_1_MODEL_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] GPT-4.1 model created${NC}"
...
if ! echo "$GPT35_MODEL_RESPONSE" | grep -q '"key":"gpt-3.5-turbo"'; then
    echo -e "${RED}ERROR: Failed to create GPT-3.5-turbo model${NC}"
    echo "Response: $GPT35_MODEL_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] GPT-3.5-turbo model created${NC}"
...
if ! echo "$AZURE_GPT4_MODEL_RESPONSE" | grep -q '"key":"gpt-4o-azure"'; then
    echo -e "${RED}ERROR: Failed to create Azure GPT-4 model${NC}"
    echo "Response: $AZURE_GPT4_MODEL_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] Azure GPT-4 model created${NC}"
...
if ! echo "$CHAT_PIPELINE_RESPONSE" | grep -q '"name":"default"'; then
    echo -e "${RED}ERROR: Failed to create chat pipeline${NC}"
    echo "Response: $CHAT_PIPELINE_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] Chat pipeline created${NC}"
...
if ! echo "$SIMPLE_PIPELINE_RESPONSE" | grep -q '"name":"simple-pipeline"'; then
    echo -e "${RED}ERROR: Failed to create simple pipeline${NC}"
    echo "Response: $SIMPLE_PIPELINE_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] Simple pipeline created${NC}"
