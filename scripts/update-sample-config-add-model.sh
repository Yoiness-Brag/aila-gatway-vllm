...
if [ -z "$OPENAI_PROVIDER_ID" ]; then
    echo -e "${RED}ERROR: OpenAI provider not found${NC}"
    echo -e "${YELLOW}Please run create-sample-config.sh first to create the base configuration${NC}"
    exit 1
fi
echo -e "${GREEN}   [OK] OpenAI provider found (ID: $OPENAI_PROVIDER_ID)${NC}"
...
if echo "$MODELS_RESPONSE" | grep -q '"key":"gpt-4.1-mini"'; then
    echo -e "${YELLOW}   [WARN] GPT-4.1 mini model already exists, skipping creation${NC}"
...
if [ "$GPT4_MINI_EXISTS" = false ]; then
    ...
    if ! echo "$GPT4_MINI_MODEL_RESPONSE" | grep -q '"key":"gpt-4.1-mini"'; then
        echo -e "${RED}ERROR: Failed to create GPT-4.1 mini model${NC}"
        echo "Response: $GPT4_MINI_MODEL_RESPONSE"
        exit 1
    fi
    echo -e "${GREEN}   [OK] GPT-4.1 mini model created${NC}"
else
    echo -e "${GREEN}   [OK] GPT-4.1 mini model already exists${NC}"
fi
...
if [ -z "$DEFAULT_PIPELINE_ID" ]; then
    echo -e "${RED}ERROR: Default pipeline not found${NC}"
    echo -e "${YELLOW}Please run create-sample-config.sh first to create the base configuration${NC}"
    exit 1
fi
echo -e "${GREEN}   [OK] Default pipeline found (ID: $DEFAULT_PIPELINE_ID)${NC}"
...
if echo "$CURRENT_PIPELINE" | grep -q '"key":"gpt-4.1-mini"'; then
    echo -e "${YELLOW}   [WARN] GPT-4.1 mini is already in the default pipeline${NC}"
...
if ! echo "$UPDATE_PIPELINE_RESPONSE" | grep -q '"name":"default"'; then
    echo -e "${RED}ERROR: Failed to update default pipeline${NC}"
    echo "Response: $UPDATE_PIPELINE_RESPONSE"
    exit 1
fi
echo -e "${GREEN}   [OK] Default pipeline updated with GPT-4.1 mini${NC}"
