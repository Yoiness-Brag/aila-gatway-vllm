...
impl OtelTracer {
    pub fn init(endpoint: String, public_key: String, secret_key: String) {
        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            handle.spawn(async move {
                let result = tokio::task::spawn_blocking(move || {
                    ...
                    headers.insert("x-langfuse-sdk-name".to_string(), "rust-gateway".to_string());
                    headers.insert("x-langfuse-sdk-version".to_string(), "1.0.0".to_string());
                    headers.insert("x-langfuse-public-key".to_string(), public_key.clone());
                    
                    let full_endpoint = format!("{}/api/public/otel/v1/traces", endpoint.trim_end_matches('/'));

                    let exporter_result = SpanExporter::builder()
                        .with_http()
                        .with_endpoint(full_endpoint.clone())
    ...
    pub fn start<T: RecordSpan>(operation: &str, request: &T) -> Self {
        let tracer = global::tracer("langfuse");
        let mut span = tracer
            .span_builder(format!("langfuse.{operation}"))
