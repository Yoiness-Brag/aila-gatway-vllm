...
pub fn create_pipeline(pipeline: &Pipeline, model_registry: &ModelRegistry) -> Router {
    ...
    for plugin in pipeline.plugins.clone() {
        router = match plugin {
            PluginConfig::Tracing { endpoint, public_key, secret_key } => {
                tracing::info!("Initializing Langfuse OtelTracer for pipeline {}", pipeline.name);
                OtelTracer::init(endpoint, public_key, secret_key);
                router
            }
...
pub async fn chat_completions(
    ...
    for model_key in model_keys {
        ...
        if payload.model == model.name {
            ...
            if let ChatCompletionResponse::NonStream(mut completion) = response {
                completion.model = payload.model.clone();
...
pub async fn completions(
    ...
    for model_key in model_keys {
        ...
        if payload.model == model.name {
            ...
            let mut response = model.completions(payload.clone()).await.inspect_err(|e| {
                eprintln!("Completion error for model {model_key}: {e:?}");
            })?;
            response.model = payload.model.clone();
...
pub async fn embeddings(
    ...
    for model_key in model_keys {
        ...
        if payload.model == model.name {
            ...
            let mut response = model.embeddings(payload.clone()).await.inspect_err(|e| {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri("/models")
                    .method("GET")
                    .header("x-langfuse-pipeline", "test")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
