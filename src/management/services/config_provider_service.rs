        ...
        match dto.plugin_type {
            ...
            super::super::dto::PluginType::Tracing => {
                ...
                let resolved_public_key = self
                    .secret_resolver
                    .resolve_secret(&tracing_config.public_key)
                    .await?;

                let resolved_secret_key = self
                    .secret_resolver
                    .resolve_secret(&tracing_config.secret_key)
                    .await?;
...
mod tests {
    ...
    fn test_tracing_config_dto_deserialization() {
        ...
        assert_eq!(
            config.public_key,
            SecretObject::literal("pk-lf-test".to_string())
        );
        assert_eq!(
            config.secret_key,
            SecretObject::literal("sk-lf-test".to_string())
        );
    }
